...
ap.add_argument("-ck_steps", "--chekpoint_steps", required=True,  nargs='+',   help="Epochs evaluated list of int. ")
ap.add_argument("-scorepathFM", "--score_output_pathFM", required=False, default='path/to/save/scores_fm_map',  help="Path where the anomaly maps from the feature maps will be saved.")
ap.add_argument("-scorepathIM", "--score_output_pathIM", required=False, default='path/to/save/scores_im_map',  help="Path where the anomaly maps from the images will be saved.")
...
ap.add_argument("-reconsIM_Path", "--reconsIM_Path", required=False, default='path/to/reconstructed_images',   help="Path where the reconstructed images will be saved. ")
ap.add_argument("-reconsFM_Path", "--reconsFM_Path", required=False, default='path/to/reconstructed_feature_maps',   help="Path where the reconstructed feature maps will be saved. ")
...
class TestMvTec:
    ...
    def data_loader(self):
        ...
        # train_dset = MVTECTestDataLoader(
        #                                  root = self.data_root_directory,
        #                                  set_ = 'Train',
        #                                  transform_process= test_transform)
    ...
    def inference(self, step, test_set = True, write_loss = False):
        ...
        df_main = pd.DataFrame({'fname':[], 'Loss':[], 'cls':[], 'MSEFM':[], 'SSIMFM':[], 'MSEIM':[], 'SSIMIM':[], 'Logit1':[], 'Logit2':[]})
        # %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        # Run inference
        ...
        df = pd.DataFrame({'fname':[], 'Loss':[], 'cls':[], 'MSEFM':[], 'SSIMFM':[], 'MSEIM':[], 'SSIMIM':[], 'Logit1':[], 'Logit2':[]})
...
if __name__=="__main__":
    # res_dir_full_path =os.path.join( args['data_root_directory'], 'res')
